...
class Cos extends Driver
{
    ...
    public function __construct(array $options = [])
    {
        ...
        if (!$this->options['upload_access_id'] || !$this->options['upload_secret_key'] || !$this->options['upload_bucket']) {
            throw new InvalidArgumentException('Configure the upload parameters of Tencent Cos storage first!');
        }
    }
    ...
    {
        return new Client([
            'region'      => $this->options['upload_url'],
            'credentials' => [
                'secretId'  => $this->options['upload_access_id'],
                'secretKey' => $this->options['upload_secret_key']
            ]
        ]);
    }
    ...
    public function save(UploadedFile $file, string $saveName): bool
    {
        ...
        $client->upload($this->options['upload_bucket'], $saveName, fopen($file->getPathname(), 'rb'));
        return true;
    }
    ...
    public function delete(string $saveName): bool
    {
        ...
        $client   = $this->getClient();
        $client->deleteObject([
            'Bucket' => $this->options['upload_bucket'],
            'Key'    => $saveName,
        ]);
        return true;
    }
    ...
    {
        if ($domain === true) {
            $bucketUrl = 'https://' . $this->options['upload_bucket'] . '.cos.' . $this->options['upload_url'] . '.myqcloud.com';
    ...
    public function exists(string $saveName): bool
    {
        $saveName = $this->handleSaveName($saveName);
        $client   = $this->getClient();
        return $client->doesObjectExist($this->options['upload_bucket'], $saveName);
    }
}